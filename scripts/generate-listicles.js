 ...
 *
 * Creates the "Recommends" guide pages by walking every category x region
  ...
  for (const candidate of eligible.slice(0, limit)) {
    ...
    const listicle = {
      ...
      filters: {
        ...
        // was written for instead sorted by event date; hotels have no
        // date field, so that default doesn't apply here.)
        sort_by: 'sentiment_scores.overall',
        sort_direction: 'desc',
        limit: 15,
      },
      manual_entity_ids: [],
