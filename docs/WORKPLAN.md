...
- **Site:** elitestay.travel — programmatic-SEO directory of Marriott Bonvoy hotels across 12 SE-Asia cities spanning 6 countries (Singapore, Bangkok, Phuket, Kuala Lumpur, Bali, Ho Chi Minh City, Jakarta, Penang, Da Nang, Manila, Ko Samui, Surabaya). Astro, image-free, tier-coded typographic cards (see `src/lib/brand.js`).
...
- ✅ **WS2 done** (2026-08-05) — external-coverage backfill complete via the `scripts/coverage/` pipeline (subagent → verify → insert). **116/119 entities now carry verified `external_coverage`.** The 3 without have no independent coverage that exists yet — `courtyard-phuket-chalong-bay` (opened Mar 2026), `marriott-executive-apartments-bangkok-sukhumvit-50` (long-stay, only OTAs/TripAdvisor) — or are unpublished (`jw-marriott-bali-ubud` draft). Two hotels are video-only (no independent written review found): `marriott-executive-apartments-bangkok-sukhumvit-park`, `metropole-bangkok-tribute-portfolio`. Curation dropped pre-rebrand reviews (Four Points Ploenchit's Novotel-era review; Metropole's pre-Tribute review), rental-listing/GM-promo videos, and same-blog restaurant-only filler (Courtyard KL South).
- ✅ **WS5 full manual sweep run (2026-08-05)** — replicated `audit-entities.js` logic across all **117 published** entities via 10 subagents (same conservative criteria; `audit_last_checked: 2026-08-05` stamped on all). Result: 115 open, 1 uncertain, **1 left-Marriott**.
  - 🚩 **`le-meridien-suvarnabhumi-bangkok-golf-resort-spa` → set `needs_review`**: reflagged as **Hilton Bangkok Suvarnabhumi Golf Resort & Spa on 1 Jan 2026** (Marriott management rights expired) — source [LoyaltyLobby 2025-11-11](https://loyaltylobby.com/2025/11/11/le-meridien-suvarnabhumi-airport-hotel-reflagged-as-hilton-on-january-1-2026/). **Human must confirm → archive** (its WS2 coverage is now moot). This is the exact Le-Méridien-Sentosa-style drift; the sweep caught it.
  - ✅ `four-points-bangkok-sukhumvit-15` **CLEARED** — the FlyerTalk "leaves Bonvoy" rumor was wrong; still Four Points, bookable, only a temp Jul–Aug 2026 renovation.
  - `westin-siray-bay-phuket` = `uncertain` (the known Ritz conversion; not flagged, still Marriott). `veranda-resort-phuket-autograph-collection` Cape-Panwa location fix still open (spawned task).
- ✅ **City expansion (2026-08-05):** added **Penang (9)** and **Jakarta (19)** as tracked cities — 28 new full entities with hand-judged scores + verified coverage. Both parent countries (Malaysia/Indonesia) already existed as regions; added `data/regions/penang.json` + `data/regions/jakarta.json` (granularity city), broadened the Malaysia/Indonesia country intros, updated `regionsSupported` (now 8 cities; Jakarta re-added after being dropped 2026-08-03). Catalogue **119 → 147**. Discovery = marriott.com sweep per city (verified marsha codes). Jakarta includes the Greater-Jakarta Bekasi satellites + the Ritz-Carlton Residences. `needs_review` (very new, no/thin coverage): `aloft-jakarta-kebon-jeruk`, `four-points-bekasi`. Not-yet-open excluded: Westin Penang, Courtyard Penang Icon City. Reusable pipeline proven: discover → research subagents → summary-shortener subagent → **assembler subagent** (research JSON → entity JSON via a templated spec) → verify/insert coverage → commit region+entities together (never deploy an empty city page).
- ✅ **City expansion (2026-08-06):** added **Da Nang (7)** and **Manila (5)** as tracked cities — 12 new full entities with hand-judged scores + verified coverage (41 reviews + 34 videos, oEmbed/HTTP gate passed). Vietnam parent already existed (added `data/regions/da-nang.json`, granularity city, and broadened the Vietnam country intro to cover the Da Nang–Hoi An coast). **Philippines is a brand-new 6th country:** added `data/regions/philippines.json` (country) + `data/regions/manila.json` (city). `regionsSupported` now 10 cities. Catalogue **147 → 159**. Da Nang set = Renaissance Danang Hoi An (ex-Vinpearl), Courtyard Han River, Danang Marriott Resort + its separate Non Nuoc Beach Villas listing, Sheraton Grand, Four Points, MEA Han River. Manila set = Sheraton Manila Bay (ex-Pan Pacific), The Westin Manila (Ortigas), AC Hotel Manila (Ortigas/Pasig), Manila Marriott + Sheraton Manila at Newport World Resorts. **Excluded:** Hoi An Marriott (opening Oct 2026) and The Farm at San Benito (Autograph, but 78 km south in Batangas — not Metro Manila; candidate for a future Batangas/Tagaytay page). `needs_review` (very new, thin/no coverage): `ac-hotel-manila` (Dec 2025), `marriott-executive-apartments-danang-han-river` (2026, no coverage exists yet). Same reusable NEW-CITY pipeline as Penang/Jakarta; assembly done via a deterministic Node assembler (no LLM transcription) since the dev machine now has Node 24 but not the project deps (zod), so `npm run validate` still can't run locally — validated instead with a dependency-free standalone checker replicating the schema/ref rules.
- ✅ **City expansion (2026-08-06, batch 2):** added **Ko Samui (5)** and **Surabaya (7)** as tracked cities — 12 new full entities with hand-judged scores + verified coverage (38 reviews + 46 videos, oEmbed/HTTP gate passed). Both parent countries already existed: added `data/regions/ko-samui.json` (parent thailand) + `data/regions/surabaya.json` (parent indonesia); broadened the Thailand + Indonesia country intros. `regionsSupported` now 12 cities. Catalogue **159 → 171**. Ko Samui set (all beach resorts, skews luxury) = Vana Belle (Luxury Collection), W Koh Samui, Ritz-Carlton, Renaissance, Sheraton Samui. Surabaya set = JW Marriott, Westin, Sheraton, two Four Points (Tunjungan Plaza central + Pakuwon Indah west — kept distinct), Fairfield, Aloft Pakuwon City. `needs_review`: `aloft-surabaya-pakuwon-city` (opened Aug 2025 — only opening-press coverage exists, no experiential reviews yet). Note: the OLD JW Marriott Surabaya was itself the original "Westin Surabaya" (1996, rebranded JW 2002) — unrelated to today's new-build The Westin Surabaya (2020, Pakuwon); no conflict. Same deterministic-assembler pipeline as batch 1.
- ✅ **SEA full buildout (2026-08-06/07) — COMPLETE, 56/56 hotels.** Final 5 (Siem Reap, Bintan x2, Yogyakarta, Solo) landed in commit `770a209` after the usage limit eased. Catalogue **171 → 227**; 35 cities / 7 countries covering essentially all currently-open Marriott Bonvoy SEA properties. Coverage backfill for the 3 initially-bare hotels (`courtyard-phnom-penh`, `four-points-boracay`, `fairfield-cebu-mactan`) also DONE (commit `6195893`) — every SEA hotel now carries verified external_coverage except where none genuinely exists yet (fairfield-cebu-mactan is video-only). Nothing outstanding on the SEA buildout.
- 〜 _(historical) SEA buildout wave detail — 51 of 56 done, 5 remaining:_ Added the rest of Marriott's SEA footprint in 5 waves (deterministic assembler + verified-coverage pipeline). Catalogue **171 → 222**. Live commits: `63b309c` Vietnam (Nha Trang/Hanoi/Phu Quoc, 10), `84fbfbe` Malaysia (Kota Kinabalu/Langkawi/Johor Bahru/Desaru, 13), `1349db9` Indonesia (Bandung/Lombok/Batam, 12), `d235b68` Thailand (Pattaya/Hua Hin/Rayong/Chiang Mai/Krabi, 9), `aaed198` Philippines+Cambodia partial (Cebu/Boracay/Batangas/Phnom Penh, 7). **Cambodia added as 7th country.** Now 31 tracked cities / 7 countries.
  - **⛔ BLOCKED by weekly usage limit (resets Aug 10, 11am Asia/Singapore).** Wave 5 was cut off mid-run.
  - **RESUME AFTER AUG 10 — build these 5 hotels** (same pipeline; SPEC + assembler were in the session scratchpad which is now gone, so re-create per the WS-city recipe above): **Siem Reap** → `courtyard-siem-reap` (Courtyard by Marriott Siem Reap Resort); **Bintan** → `four-points-bintan` (Four Points by Sheraton Bintan, Lagoi Bay) + `natra-bintan-tribute` (Natra Bintan, a Tribute Portfolio Resort); **Yogyakarta** → `yogyakarta-marriott` (Yogyakarta Marriott Hotel, Marriott Hotels); **Solo** → `moxy-solo` (Moxy Solo, Moxy).
  - **Region files `data/regions/{siem-reap,bintan,yogyakarta,solo}.json` are already written but UNCOMMITTED** in the working tree (not committed because their city pages would be empty until the hotels land). Commit each WITH its entities. cambodia.json is already committed (parent of both phnom-penh and siem-reap).
  - **3 hotels shipped coverage-less** (curation cut off): `courtyard-phnom-penh`, `four-points-boracay`, `fairfield-cebu-mactan` — backfill external_coverage via `scripts/coverage/` when convenient.
  - `site.config.json` `regionsSupported` + notes still list only 12 cities — update to the full 31 (housekeeping; does not affect live pages, which build from data/regions/*).
- ⬜ Remaining: **WS6** (affiliate monetization).
- 🐞 **Latent data-quality issue (found 2026-08-06, NOT yet fixed):** ~6 existing entities have `short_description` > the schema's 220-char max (e.g. `ritz-carlton-residences-jakarta-pacific-place` 242, `the-millen-penang-autograph-collection` 335, `sari-pacific-jakarta-autograph-collection` 245, `the-hermitage-jakarta-tribute-portfolio` 249, `the-prestige-penang-design-hotels` 285, plus several Jakarta/Penang ~222–237). They shipped because the manual add-path deploy (`deploy.yml`) only runs `npm run build`, not `npm run validate`. But **weekly-refresh.yml runs `npm run validate` as its pre-flight step** — so the next automated weekly refresh will FAIL at that step and never discover/commit new hotels until these are trimmed to ≤220. Fix: shorten those `short_description`s. (My 12 new entities are all ≤220.)
