...
- ⚠️ **Audit flag (WS5-type):** a coverage subagent surfaced that **`four-points-bangkok-sukhumvit-15` reportedly leaves Marriott Bonvoy in Jan 2026** (FlyerTalk). Unverified — needs the WS3/WS5 human-confirm treatment (verify on marriott.com; if it left, set `needs_review`/archive). Also `veranda-resort-phuket-autograph-collection` may be mislocated (Cape Panwa, not Mai Khao — see spawned task).
- ⬜ Remaining: **WS6** (affiliate monetization). Optional: run `npm run audit` (WS5) to sweep for more closed/left-Marriott properties like the FP Sukhumvit 15 flag.
